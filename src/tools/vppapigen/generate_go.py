...
def validate_args(vpp_dir, o, f, c, i):
    if o is None:
        o = vpp_dir
    elif not os.path.exists(o) or os.path.isfile(o):
        print(f"{o} is not a valid output path")
        sys.exit(1)
    if f is None:
        f = []
    if c is None:
...
def install_golang(go_root):
    ...
    if os.path.exists(go_bin) and os.path.isfile(go_bin):
        print(f'Go {get_go_version(go_root)} is already installed')
        return
    ...
    go_tf = tarfile.open(f"/tmp/{filename}")
    ...
    print(f'Go {get_go_version(go_root)} was installed')


# Installs latest binary API generator
def install_binapi_gen(c, go_root, go_path):
    os.environ['GO111MODULE'] = "on"
    if os.path.exists(f"{go_root}/bin/go") & os.path.isfile(
        f"{go_root}/bin/go"
    ):
        p = subprocess.Popen(
            ["./go", "get", f"git.fd.io/govpp.git/cmd/binapi-generator@{c}"],
            cwd=f"{go_root}/bin",
            stdout=subprocess.PIPE,
    ...
    print(f'Installed binary API generator {bg_ver}')
...
def generate_api(output_dir, vpp_dir, api_list, import_prefix, no_source, go_path):
    output_binapi = (
        f"{output_dir}binapi"
        if output_dir[-1] == "/"
        else f"{output_dir}/binapi"
    )

    json_dir = f"{vpp_dir}/build-root/install-vpp-native/vpp/share/vpp/api"
    ...
    cmd = [
        "./binapi-generator",
        f"--output-dir={output_binapi}",
        f"--input-dir={json_dir}",
    ]
    ...
    if len(api_list):
        ...
        cmd.append(api_list)
    if import_prefix != "":
        cmd.append(f"-import-prefix={import_prefix}")
    if no_source:
    ...
    print("\n")
    print(f"Go API bindings were generated to {output_binapi}")
...
def main():
    ...
    generate_api(o, vpp_dir, f, i, args.no_source_path_info, go_path)
